...
export class GratitudesService {
  ...
  public async create(authUser: AuthUser, data: CreateGratitudeDto) {
    ...
    const result = await this.repository.findOneOrFail(id, { relations: ['fromUser', 'toUser', 'course'] });
    await this.postToDiscord(result);
    return result;
  }

  public getBadges({ courses }: AuthUser, courseId: number) {
  ...
  private postToDiscord(feedback: Feedback) {
    if (!feedback.fromUser || !feedback.toUser || !feedback.comment || !feedback.course?.discordServer?.gratitudeUrl) {
      this.logger.log('No fromUser or toUser or comment or gratitudeUrl');
      return Promise.resolve(null);
    }
    return this.discordService.pushGratitude({
